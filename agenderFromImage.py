# -*- coding: utf-8 -*-
"""
Created on Wed Apr 22 11:48:59 2020
...
if (len(face_boxes) > 0):
    ...
    for (label, box) in zip(labels, face_boxes):
        cv.putText(faces_bgr, label, org=(box[0], box[1] +5), fontFace=cv.FONT_HERSHEY_PLAIN,
                   fontScale=0.8, color=(0, 255, 255), thickness=2, lineType=cv.LINE_AA)
elapse_time = time.time() - time_starting 
print("time", elapse_time)
# Show frames
cv.putText(faces_bgr,"time:"+str(elapse_time), (10,15), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1, cv.LINE_AA)

cv.imshow('Faces', faces_bgr)

filename = 'savedImage.png'
  
#Using cv.imwrite() method 
# Saving the image 
cv.imwrite(filename, faces_bgr)
